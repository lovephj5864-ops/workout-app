...
with tab_mad:
    ...
    mon_back = mc1.selectbox("등", acc_dict["등"], index=acc_dict["등"].index("펜들레이로우" if "펜들레이로우" in acc_dict["등"] else acc_dict["등"][0]))
    mon_core = mc2.selectbox("코어/복근", acc_dict["코어/복근"], index=acc_dict["코어/복근"].index("플랭크" if "플랭크" in acc_dict["코어/복근"] else acc_dict["코어/복근"][0]))

    st.markdown("**🟨 수요일 보조**")
    wc1, wc2, wc3, wc4 = st.columns(4)
    wed_sh = wc1.selectbox("어깨", acc_dict["어깨"], index=acc_dict["어깨"].index("숄더 프레스" if "숄더 프레스" in acc_dict["어깨"] else acc_dict["어깨"][0]))
    wed_quad = wc2.selectbox("대퇴사두", acc_dict["대퇴사두"], index=acc_dict["대퇴사두"].index("핵스쿼트" if "핵스쿼트" in acc_dict["대퇴사두"] else acc_dict["대퇴사두"][0]))
    wed_bi = wc3.selectbox("이두", acc_dict["이두"], index=acc_dict["이두"].index("바벨 이두컬" if "바벨 이두컬" in acc_dict["이두"] else acc_dict["이두"][0]))
    wed_tri = wc4.selectbox("삼두", acc_dict["삼두"], index=acc_dict["삼두"].index("클로즈그립 푸시업" if "클로즈그립 푸시업" in acc_dict["삼두"] else acc_dict["삼두"][0]))

    st.markdown("**🟦 금요일 보조**")
    fc1, fc2 = st.columns(2)
    fri_back = fc1.selectbox("등", acc_dict["등"], index=acc_dict["등"].index("풀업" if "풀업" in acc_dict["등"] else acc_dict["등"][0]))
    fri_post = fc2.selectbox("후면사슬", acc_dict["후면사슬"], index=acc_dict["후면사슬"].index("루마니안데드리프트" if "루마니안데드리프트" in acc_dict["후면사슬"] else acc_dict["후면사슬"][0]))

    if st.button(f"🚀 {target_week}주차 루틴 생성", type="primary", use_container_width=True):
        ...
        inc = 2.5 * (target_week - 1)
        cur_sq, cur_bp, cur_dl = sq_start + inc, bp_start + inc, dl_start + inc
        ...
        tag = current_user if current_user else '기본'
        save_routine_to_sheet(f"[매드프로페서] {target_week}주차 월요일 ({tag})", mon_routine)
        save_routine_to_sheet(f"[매드프로페서] {target_week}주차 수요일 ({tag})", wed_routine)
        save_routine_to_sheet(f"[매드프로페서] {target_week}주차 금요일 ({tag})", fri_routine)
        st.success(f"🎉 생성 완료!")
...
with tab_manage:
    ...
    if selected_exs:
        for ex in selected_exs:
            ...
            sets = c2.number_input("세트", min_value=1, value=4, key=f"set_{ex}", label_visibility="collapsed")
...
with tab_workout:
    if not current_user:
        st.warning("⚠️ 상단에 [사용자 닉네임]을 입력하셔야 기록을 불러올 수 있습니다.")
    elif not st.session_state.routines:
        st.warning("등록된 루틴이 없습니다.")
    else:
        if 'last_completed_time' not in st.session_state:
            st.session_state.last_completed_time = 0

        visible_workout_routines = [r for r in st.session_state.routines.keys() if "[매드프로페서]" not in r or (current_user and f"({current_user})" in r)]
        ...
        if not visible_workout_routines:
            ...
            rest_sec = col_rest.number_input("⏱️ 휴식(초)", min_value=0, value=st.session_state.get('rest_sec_pref', 60), step=10)
            ...
            db_vol_today = db_vol_week = db_vol_month = current_unsaved_vol = 0
            today_str = datetime.now().strftime("%Y-%m-%d")
            this_ym = datetime.now().strftime("%Y-%m")
            this_yw = f"{datetime.now().isocalendar()[0]}-{datetime.now().isocalendar()[1]}"
            
            if not past_logs_df.empty and '사용자' in past_logs_df.columns:
                u_logs = past_logs_df[(past_logs_df['사용자'] == current_user) & (past_logs_df['완료여부'] == 'O')].copy()
                if not u_logs.empty:
                    u_logs['날짜_dt'] = pd.to_datetime(u_logs['날짜'])
                    u_logs['볼륨'] = u_logs['무게'] * u_logs['횟수']
                    db_vol_today = u_logs[u_logs['날짜'] == today_str]['볼륨'].sum()
                    db_vol_month = u_logs[u_logs['날짜_dt'].dt.strftime('%Y-%m') == this_ym]['볼륨'].sum()
                    u_logs['yw'] = u_logs['날짜_dt'].dt.isocalendar().year.astype(str) + "-" + u_logs['날짜_dt'].dt.isocalendar().week.astype(str)
                    db_vol_week = u_logs[u_logs['yw'] == this_yw]['볼륨'].sum()
                ...
                for w_idx, workout in enumerate(st.session_state.active_workout):
                    with st.expander(f"🔥 {workout['name']}", expanded=True):
                        ...
                        if ctrl1.button("➕ 세트추가", key=f"add_{w_idx}", use_container_width=True):
                            ...
                            st.rerun()
                        if ctrl2.button("➖ 세트삭제", key=f"sub_{w_idx}", use_container_width=True) and workout['target_sets'] > 1:
                            workout['target_sets'] -= 1
                            st.rerun()
                        if ctrl3.button("✅ 전체완료", key=f"all_{w_idx}", use_container_width=True):
                            st.session_state[f"mark_all_{w_idx}"] = True
